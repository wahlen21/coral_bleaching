...
ggplot(df, aes(x = LAT, y = LON, fill = BLEACHING_SEVERITY)) + 
  geom_density_2d() + 
...
# Hexbin plot
ggplot(top_5_countries_df, aes(x = LAT, y = LON, fill = BLEACHING_SEVERITY)) + 
  geom_hex(bins = 30, alpha = 0.9, color='black') +  # Set number of bins and adjust transparency
  facet_wrap(~COUNTRY, scales = "free") +  # Allow each facet to have its own scales
  labs(
